...
let unionTypeStrings ::types () =>
  "(["
  ^ (List.map (fun type_name => "`" ^ type_name) types |> String.concat " | ")
  ^ "] [@bs.string])";
...
let inlineUnion ::types () =>
  ...
  ^ (
    List.map
      (fun (type_name, type_of) => "`" ^ type_name ^ " " ^ type_of) types
    |> String.concat " | "
  )
  ^ "] [@bs.unwrap])";
...
let react_component ::module_name ::component_name ::js_name ::props () =>
  ...
  ^ (
    List.map
      (
        fun (name, js, t, optional, is_bool) =>
          "\""
          ^ js
          ^ "\": "
          ^ (is_bool ? "Js.Boolean.to_js_boolean " : "")
          ^ name
      )
      props
    |> String.concat ", "
